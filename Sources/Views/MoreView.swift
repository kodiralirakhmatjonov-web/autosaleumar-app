...
struct MoreView: View {
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    nativeSection(L10n.t("Выбор автомобиля", "Avtomobil tanlash", settings.language)) {
                        row("sparkles", L10n.t("Персональный подбор", "Shaxsiy tanlov", settings.language)) { showRequest = true }
                        Divider().padding(.leading, 54)
                        row("arrow.left.arrow.right", L10n.t("Сравнение и Smart-Консультант", "Solishtirish va Smart-Maslahatchi", settings.language)) { showCompare = true }
                        Divider().padding(.leading, 54)
                        row("calendar", L10n.t("Забронировать визит", "Tashrifni band qilish", settings.language)) { showBooking = true }
                    }

                    nativeSection(L10n.t("Auto Sale Umar", "Auto Sale Umar", settings.language)) {
                        navRow("location.fill", L10n.t("Локация шоурума", "Shourum lokatsiyasi", settings.language), destination: LocationView())
                        Divider().padding(.leading, 54)
                        navRow("checkmark.shield.fill", L10n.t("25 лет доверия", "25 yil ishonch", settings.language), destination: TrustView(openCatalog: { selectTab(.catalog) }))
                        Divider().padding(.leading, 54)
                        navRow("gift.fill", "Ramadan Gift", destination: RamadanGiftView())
                    }
    ...
    private func navRow<Destination: View>(_ symbol: String, _ title: String, destination: Destination) -> some View {
        NavigationLink(destination: destination) { rowLabel(symbol, title) }.buttonStyle(.plain)
    }
